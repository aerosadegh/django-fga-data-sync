...
1. When a Django model is saved or deleted, the `FGASyncMixin` intercepts the action.
...
*   **Zero-Boilerplate Synchronization:** By simply adding the `FGASyncMixin` to a model and defining the `FGA_SETTINGS` dictionary, the package automatically calculates tuple diffs on every `save()` or `delete()` and syncs them to OpenFGA.

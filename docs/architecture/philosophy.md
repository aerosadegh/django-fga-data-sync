...
*   **Zero-Boilerplate Synchronization:** By simply adding the `FGAModelSyncMixin` to a model and defining the `FGA_SETTINGS` dictionary, the package automatically calculates tuple diffs on every `save()` or `delete()` and syncs them to OpenFGA.
